...
static bool edt_rpi_ft5x06_ts_check_crc(struct edt_rpi_ft5x06_ts_data *tsdata,
				    u8 *buf, int buflen)
{
	int i;
	u8 crc = 0;
...
static irqreturn_t edt_rpi_ft5x06_ts_isr(int irq, void *dev_id)
{
	struct edt_rpi_ft5x06_ts_data *tsdata = dev_id;
	...
	error = edt_rpi_ft5x06_ts_readwrite(tsdata->client,
					sizeof(cmd), &cmd,
					datalen, rdbuf);
	if (error) {
...
static void edt_rpi_ft5x06_ts_irq_poll_timer(struct timer_list *t)
{
	struct edt_rpi_ft5x06_ts_data *tsdata = from_timer(tsdata, t, timer);

	schedule_work(&tsdata->work_i2c_poll);
	mod_timer(&tsdata->timer, jiffies + msecs_to_jiffies(POLL_INTERVAL_MS));
}

static void edt_rpi_ft5x06_ts_work_i2c_poll(struct work_struct *work)
{
	struct edt_rpi_ft5x06_ts_data *tsdata = container_of(work,
			struct edt_rpi_ft5x06_ts_data, work_i2c_poll);

	edt_rpi_ft5x06_ts_isr(0, tsdata);
}

static int edt_ft5x06_register_write(struct edt_rpi_ft5x06_ts_data *tsdata,
				     u8 addr, u8 value)
{
	u8 wrbuf[4];
	...
	case EDT_M06:
		...
		return edt_rpi_ft5x06_ts_readwrite(tsdata->client, 4,
					wrbuf, 0, NULL);
	case EDT_M09:
	case EDT_M12:
	case GENERIC_FT:
		...
		return edt_rpi_ft5x06_ts_readwrite(tsdata->client, 2,
					wrbuf, 0, NULL);
...
static int edt_ft5x06_register_read(struct edt_rpi_ft5x06_ts_data *tsdata,
				    u8 addr)
{
	u8 wrbuf[2], rdbuf[2];
	int error;
	...
	case EDT_M06:
		...
		error = edt_rpi_ft5x06_ts_readwrite(tsdata->client, 2, wrbuf, 2,
						rdbuf);
		if (error)
			return error;
	...
	case GENERIC_FT:
		wrbuf[0] = addr;
		error = edt_rpi_ft5x06_ts_readwrite(tsdata->client, 1,
						wrbuf, 1, rdbuf);
		if (error)
			return error;
		break;
...
#define EDT_ATTR(_field, _mode, _addr_m06, _addr_m09,			\
	...
	struct edt_ft5x06_attribute edt_ft5x06_attr_##_field = {	\
		...
		.field_offset = offsetof(struct edt_rpi_ft5x06_ts_data, _field), \
		.addr_m06 = _addr_m06,					\
		.addr_m09 = _addr_m09,					\
				       ...
				       char *buf)
{
	...
	struct edt_rpi_ft5x06_ts_data *tsdata = i2c_get_clientdata(client);
...
static inline int edt_ft5x06_write_delay(struct edt_rpi_ft5x06_ts_data *tsdata,
				   u16 wr_len, u8 *wr_buf,
				   u16 delay)
{
	...
	ret = edt_rpi_ft5x06_ts_readwrite(client, wr_len, wr_buf, 0, NULL);
	if (ret < 0) {
...
static inline bool edt_ft5x06_wait_tp_to_valid(
			struct edt_rpi_ft5x06_ts_data *tsdata)
{
	struct i2c_client *client = tsdata->client;
	int ret;
	int cnt = 0;
	...
	do {
		cmd = EDT_CMD_CHIP_ID;
		ret = edt_rpi_ft5x06_ts_readwrite(client, 1, &cmd, 1, &idh);
		if (ret < 0) {
			dev_err(&client->dev, "read chip failed\n");
			return false;
		}
		cmd = EDT_CMD_CHIP_ID2;
		ret = edt_rpi_ft5x06_ts_readwrite(client, 1, &cmd, 1, &idl);
...
static int edt_ft5x06_fwupg_get_boot_state(struct edt_rpi_ft5x06_ts_data *tsdata,
				    enum edt_fw_status *fw_sts)
{
	...
	*fw_sts = EDT_RUN_IN_ERROR;
	...
	ret = edt_rpi_ft5x06_ts_readwrite(client, 4, cmd, 2, val);
	if (ret < 0) {
...
static int edt_ft5x06_reset(struct edt_rpi_ft5x06_ts_data *tsdata)
{
...
static int edt_ft5x06_fwupg_reset_to_boot(struct edt_rpi_ft5x06_ts_data *tsdata)
{
	struct i2c_client *client = tsdata->client;
	int ret;
	u8 buf[2];
...
static int edt_ft5x06_fwupg_reset_in_boot(struct edt_rpi_ft5x06_ts_data *tsdata)
{
	struct i2c_client *client = tsdata->client;
	int ret;
	u8 cmd = EDT_CMD_RESET;
...
static int edt_ft5x06_enter_bl(struct edt_rpi_ft5x06_ts_data *tsdata)
{
	struct i2c_client *client = tsdata->client;
	int ret;
	bool fwvalid;
...
static int edt_ft5x06_erase(struct edt_rpi_ft5x06_ts_data *tsdata)
{
	struct i2c_client *client = tsdata->client;
	int ret;
	u8 cmd;
...
static int edt_ft5x06_write_fw(struct edt_rpi_ft5x06_ts_data *tsdata,
...
static int edt_ft5x06_ecc_cal(struct edt_rpi_ft5x06_ts_data *tsdata)
{
	struct i2c_client *client = tsdata->client;
	int ret;
	u8 reg_val = 0;
	...
	ret = edt_rpi_ft5x06_ts_readwrite(client, 1, &cmd, 1, &reg_val);
	if (ret < 0) {
...
static int edt_ft5x06_i2c_do_update_firmware(struct edt_rpi_ft5x06_ts_data *ts,
					u32 start_addr,
...
static int edt_ft5x06_i2c_fw_update(struct edt_rpi_ft5x06_ts_data *ts)
{
	struct i2c_client *client = ts->client;
...
static int edt_ft5x06_factory_mode(struct edt_rpi_ft5x06_ts_data *tsdata)
{
	struct i2c_client *client = tsdata->client;
...
static int edt_ft5x06_work_mode(struct edt_rpi_ft5x06_ts_data *tsdata)
{
	struct i2c_client *client = tsdata->client;
	int retries = EDT_SWITCH_MODE_RETRIES;
...
{
	struct edt_rpi_ft5x06_ts_data *tsdata = data;
...
{
	struct edt_rpi_ft5x06_ts_data *tsdata = file->private_data;
	struct i2c_client *client = tsdata->client;
...
static void
edt_rpi_ft5x06_ts_prepare_debugfs(struct edt_rpi_ft5x06_ts_data *tsdata,
...
static void
edt_rpi_ft5x06_ts_teardown_debugfs(struct edt_rpi_ft5x06_ts_data *tsdata)
{
...
static int edt_rpi_ft5x06_ts_identify(struct i2c_client *client,
					struct edt_rpi_ft5x06_ts_data *tsdata,
					char *fw_version)
{
	 ...
	 */
	memset(rdbuf, 0, sizeof(rdbuf));
	error = edt_rpi_ft5x06_ts_readwrite(client, 1, "\xBB",
	...
	if (!strncasecmp(rdbuf + 1, "EP0", 3)) {
		...
		error = edt_rpi_ft5x06_ts_readwrite(client, 1, "\xA6",
						2, rdbuf);
		if (error)
			return error;
		...
		error = edt_rpi_ft5x06_ts_readwrite(client, 1, "\xA8",
						1, rdbuf);
		if (error)
			return error;
...
static void edt_rpi_ft5x06_ts_get_defaults(struct device *dev,
				       struct edt_rpi_ft5x06_ts_data *tsdata)
{
	struct edt_reg_addr *reg_addr = &tsdata->reg_addr;
	u32 val;
...
static void
edt_rpi_ft5x06_ts_get_parameters(struct edt_rpi_ft5x06_ts_data *tsdata)
{
	struct edt_reg_addr *reg_addr = &tsdata->reg_addr;
...
static void
edt_rpi_ft5x06_ts_set_regs(struct edt_rpi_ft5x06_ts_data *tsdata)
{
	struct edt_reg_addr *reg_addr = &tsdata->reg_addr;
...
static int edt_rpi_ft5x06_ts_probe(struct i2c_client *client,
					 const struct i2c_device_id *id)
{
	...
	error = edt_rpi_ft5x06_ts_identify(client, tsdata, fw_version);
	if (error) {
	 ...
	 */
	edt_rpi_ft5x06_ts_readwrite(tsdata->client, 2, buf, 2, buf);

	edt_rpi_ft5x06_ts_set_regs(tsdata);
	edt_rpi_ft5x06_ts_get_defaults(&client->dev, tsdata);
	edt_rpi_ft5x06_ts_get_parameters(tsdata);
	...
	if (client->irq) {
		...
		error = devm_request_threaded_irq(&client->dev, client->irq,
						NULL, edt_rpi_ft5x06_ts_isr, irq_flags,
						client->name, tsdata);
		if (error) {
		...
		INIT_WORK(&tsdata->work_i2c_poll,
			  edt_rpi_ft5x06_ts_work_i2c_poll);
		timer_setup(&tsdata->timer, edt_rpi_ft5x06_ts_irq_poll_timer, 0);
	...
	edt_rpi_ft5x06_ts_prepare_debugfs(tsdata, dev_driver_string(&client->dev));
...
static int edt_rpi_ft5x06_ts_remove(struct i2c_client *client)
{
	struct edt_rpi_ft5x06_ts_data *tsdata = i2c_get_clientdata(client);

	edt_rpi_ft5x06_ts_teardown_debugfs(tsdata);

	return 0;
}

static int __maybe_unused edt_rpi_ft5x06_ts_suspend(struct device *dev)
{
	struct i2c_client *client = to_i2c_client(dev);
...
static int __maybe_unused edt_rpi_ft5x06_ts_resume(struct device *dev)
{
	struct i2c_client *client = to_i2c_client(dev);
...
static SIMPLE_DEV_PM_OPS(edt_rpi_ft5x06_ts_pm_ops,
			 edt_rpi_ft5x06_ts_suspend, edt_rpi_ft5x06_ts_resume);
...
static const struct i2c_device_id edt_rpi_ft5x06_ts_id[] = {
...
MODULE_DEVICE_TABLE(i2c, edt_rpi_ft5x06_ts_id);

#ifdef CONFIG_OF
static const struct of_device_id edt_ft5x06_of_match[] = {
	{ .compatible = "pumpkin,rpi-ts", .data = &edt_ft5x06_data },
	{ /* sentinel */ }
};
MODULE_DEVICE_TABLE(of, edt_ft5x06_of_match);
#endif

static struct i2c_driver edt_rpi_ft5x06_ts_driver = {
	.driver = {
		.name = "edt_rpi_ft5x06",
		.of_match_table = of_match_ptr(edt_ft5x06_of_match),
		.pm = &edt_rpi_ft5x06_ts_pm_ops,
	},
	.id_table = edt_rpi_ft5x06_ts_id,
	.probe    = edt_rpi_ft5x06_ts_probe,
	.remove   = edt_rpi_ft5x06_ts_remove,
};

module_i2c_driver(edt_rpi_ft5x06_ts_driver);

MODULE_AUTHOR("Simon Budig <simon.budig@kernelconcepts.de>");
MODULE_DESCRIPTION("EDT RPI I2C Touchscreen Driver");
MODULE_LICENSE("GPL");
