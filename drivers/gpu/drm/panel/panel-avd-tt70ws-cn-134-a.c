...
static int avd_tt_panel_get_modes(struct drm_panel *panel)
{
	...
	panel->connector->display_info.width_mm = 106;
	panel->connector->display_info.height_mm = 224;
...
static int avd_tt_panel_add(struct avd_tt_panel *avd_tt)
{
	...
	drm_panel_init(&avd_tt->base);
	avd_tt->base.funcs = &avd_tt_panel_funcs;
	avd_tt->base.dev = &avd_tt->dsi->dev;

	return drm_panel_add(&avd_tt->base);
}
