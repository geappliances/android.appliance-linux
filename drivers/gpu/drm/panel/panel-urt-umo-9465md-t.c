...
static int umo_r_panel_get_modes(struct drm_panel *panel)
{
	...
	panel->connector->display_info.height_mm = 217;
...
static int umo_r_panel_add(struct umo_r_panel *umo_r)
{
	...
	drm_panel_init(&umo_r->base);
	umo_r->base.funcs = &umo_r_panel_funcs;
	umo_r->base.dev = &umo_r->dsi->dev;

	return drm_panel_add(&umo_r->base);
}
