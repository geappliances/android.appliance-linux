...
static struct drm_driver apu_drm_driver = {
	...
	.fops = &apu_drm_ops,
	.gem_create_object	= apu_gem_create_object,
				   ...
				   struct apu_core *apu_core)
{
	...
	drm = apu_drm->drm;
	drm->dev->iommu_group = apu_core->rproc->dev.parent->iommu_group;
	apu_drm->domain = apu_core->rproc->domain;
	set_dma_ops(drm->dev, get_dma_ops(apu_core->rproc->dev.parent));
	mask = dma_get_mask(apu_core->rproc->dev.parent);
