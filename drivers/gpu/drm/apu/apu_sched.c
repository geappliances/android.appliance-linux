...
static void apu_job_timedout(struct drm_sched_job *sched_job)
{
	struct apu_request *apu_req, *tmp;
	struct apu_job *job = to_apu_job(sched_job);
...
int apu_drm_job_init(struct apu_core *core)
{
	...
	sched = devm_kzalloc(core->dev, sizeof(*sched), GFP_KERNEL);
	if (!sched)
		return -ENOMEM;

	sched->apu_queue.fence_context = dma_fence_context_alloc(1);
	ret = drm_sched_init(&sched->apu_queue.sched,
			     &apu_sched_ops,
			     1, 0, msecs_to_jiffies(500), "apu_js");
	if (ret) {
		dev_err(core->dev, "Failed to create scheduler: %d.", ret);
		return ret;
	}

	rq = sched->apu_queue.sched.sched_rq + DRM_SCHED_PRIORITY_NORMAL;
	ret = drm_sched_entity_init(&sched->sched_entity, &rq, 1, NULL);
