...
bool mtk_driver_is_ready(void)
{
	return sMFGASYNCDev && sMFG2DDev;
}
...
enum PVRSRV_ERROR_TAG MTKDevPrePowerState(IMG_HANDLE hSysData, enum _PVRSRV_SYS_POWER_STATE_ eNewPowerState,
				 enum _PVRSRV_SYS_POWER_STATE_ eCurrentPowerState,
				 IMG_BOOL bForced)
{
	struct mtk_mfg_base *mfg_base = GET_MTK_MFG_BASE(sPVRLDMDev);
...
enum PVRSRV_ERROR_TAG MTKDevPostPowerState(IMG_HANDLE hSysData, enum _PVRSRV_SYS_POWER_STATE_ eNewPowerState,
				  enum _PVRSRV_SYS_POWER_STATE_ eCurrentPowerState,
				  IMG_BOOL bForced)
{
	struct mtk_mfg_base *mfg_base = GET_MTK_MFG_BASE(sPVRLDMDev);
