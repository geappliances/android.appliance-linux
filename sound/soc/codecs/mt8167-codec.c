...
static int mt8167_codec_valid_new_rate(struct mt8167_codec_priv *codec_data)
{
	...
	if (snd_soc_component_read(codec_data->codec, ABB_AFE_CON11) &
	...
	struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_PRE_PMU:
		...
		codec_data->pga_gain[HP_L_PGA_GAIN] =
			snd_soc_component_read(codec, AUDIO_CODEC_CON01) &
				GENMASK(2, 0);
	...
	struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_PRE_PMU:
		...
		codec_data->pga_gain[LOUT_PGA_GAIN] =
			(snd_soc_component_read(codec, AUDIO_CODEC_CON02) &
				GENMASK(12, 9)) >> 9;
		/* set to small gain for depop sequence */
		snd_soc_component_update_bits(codec,
			...
			struct mt8167_codec_priv *codec_data)
{
	/* store gain */
	codec_data->pga_gain[HP_L_PGA_GAIN] =
		snd_soc_component_read(codec_data->codec, AUDIO_CODEC_CON01) &
			GENMASK(2, 0);
	codec_data->pga_gain[HP_R_PGA_GAIN] =
		(snd_soc_component_read(codec_data->codec, AUDIO_CODEC_CON01) &
			GENMASK(5, 3)) >> 3;
...
static int mt8167_codec_micbias1_event(struct snd_soc_dapm_widget *w,
		struct snd_kcontrol *kcontrol, int event)
{
	struct snd_soc_component *codec = snd_soc_dapm_to_component(w->dapm);
	...
	case SND_SOC_DAPM_PRE_PMU:
		snd_soc_component_update_bits(codec,
				AUDIO_CODEC_CON03,
				AUDIO_CODEC_CON03_MICBIAS1_EN,
				AUDIO_CODEC_CON03_MICBIAS1_EN);
		snd_soc_component_update_bits(codec,
				AUDIO_CODEC_CON03,
				AUDIO_CODEC_CON03_UL_CODEC_EN,
				AUDIO_CODEC_CON03_UL_CODEC_EN);
		break;
	case SND_SOC_DAPM_POST_PMD:
		snd_soc_component_update_bits(codec,
				AUDIO_CODEC_CON03,
				AUDIO_CODEC_CON03_MICBIAS1_EN, 0x0);
		snd_soc_component_update_bits(codec,
				AUDIO_CODEC_CON03,
				AUDIO_CODEC_CON03_UL_CODEC_EN, 0x0);
		break;
	default:
		break;
...
static const struct snd_soc_dapm_widget mt8167_codec_dapm_widgets[] = {
	...
	SND_SOC_DAPM_SUPPLY_S("AU_MICBIAS1", 5, SND_SOC_NOPM, 0, 0,
			mt8167_codec_micbias1_event,
			SND_SOC_DAPM_PRE_PMU | SND_SOC_DAPM_POST_PMD),
			...
			size_t count, loff_t *pos)
{
	...
	for (i = 0; i < ARRAY_SIZE(mt8167_codec_dump_reg_list); i++) {
		n += scnprintf(buf + n, count - n, "%s = 0x%x\n",
			...
			snd_soc_component_read(codec_data->codec,
