			  ...
			  struct snd_pcm_substream *substream)
{
	...
	struct mt8167_afe_memif *memif = &afe->memif[dai->id];
	unsigned int hw_ptr;
	int ret;
...
static int mt8167_afe_pcm_new(struct snd_soc_component *component,
			      struct snd_soc_pcm_runtime *rtd)
{
	...
	struct mtk_afe *afe = snd_soc_component_get_drvdata(component);
	struct snd_soc_dai *dai = asoc_rtd_to_cpu(rtd, 0);
	size_t size = afe->memif[dai->id].data->prealloc_size;
	struct snd_pcm_substream *substream;
	int stream;
...
static void mt8167_afe_pcm_free(struct snd_soc_component *component,
				struct snd_pcm *pcm)
{
...
static const struct snd_soc_component_driver mt8167_afe_pcm_platform = {
	...
	.pcm_construct = mt8167_afe_pcm_new,
	.pcm_destruct = mt8167_afe_pcm_free,
			       ...
			       struct snd_soc_dai *dai)
{
	...
	if (clk_mode == MT8167_AFE_I2S_SHARED_CLOCK && snd_soc_dai_active(dai))
		return 0;
